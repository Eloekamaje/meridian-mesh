...
function Bloc({ titre, children, testid }) {
  return (
    <div data-testid={testid}>
      <div className="font-code text-[9px] uppercase tracking-[0.18em] text-[#7C93A8]">{titre}</div>
      <div className="mt-1 text-xs leading-relaxed text-[#D8E2EA]">{children}</div>
    </div>
  );
}
...
export default function CarteInitiative({ init, mesh, onChange }) {
  ...
  const g = GENRES_INIT[init.genre] || [init.genre, "#7C93A8"];
  ...
  const actions = () => {
    if (init.genre === "investigation_recommandee") {
      return (
        <>
          ...
          <button onClick={() => repondre("Créer un nouveau travail")} disabled={envoi} data-testid={`init-creer-${init.id}`} className="rounded-md border border-[rgba(148,163,184,0.16)] px-3 py-1.5 text-[11px] text-[#94A3B8] transition-colors hover:text-[#F2F6F8] disabled:opacity-50">
            Créer un nouveau travail
          </button>
          <button onClick={() => repondre("Surveiller seulement")} disabled={envoi} data-testid={`init-surveiller-${init.id}`} className="rounded-md border border-[rgba(148,163,184,0.16)] px-3 py-1.5 text-[11px] text-[#94A3B8] transition-colors hover:text-[#F2F6F8] disabled:opacity-50">
    ...
    if (init.genre === "decision_a_examiner") {
      return (
        <>
          ...
          <button onClick={() => setRejet(true)} data-testid={`init-rejeter-${init.id}`} className="rounded-md border border-[#F87171]/30 px-3 py-1.5 text-[11px] text-[#F87171] transition-colors hover:bg-[#F87171]/5">
    ...
    if (init.genre === "action_proposee") {
      return (
        <>
          <button onClick={() => navigate(`/jumeaux/${init.jumeau_cible}/revue`)} data-testid={`init-examiner-${init.id}`} className="rounded-md bg-[#F2B84B] px-3 py-1.5 text-[11px] font-semibold text-[#071019] transition-colors hover:bg-[#F8CF7A]">
    ...
    if (init.genre === "a_surveiller" || init.genre === "information") {
      return (
        <>
          ...
          <button onClick={() => repondre("Ignorer")} disabled={envoi} data-testid={`init-ignorer-${init.id}`} className="rounded-md border border-[rgba(148,163,184,0.16)] px-3 py-1.5 text-[11px] text-[#94A3B8] transition-colors hover:text-[#F2F6F8] disabled:opacity-50">
            Ignorer
          </button>
        </>
      );
    ...
    return (init.options || []).map((o) => (
      <button key={o} onClick={() => repondre(o)} disabled={envoi} data-testid={`init-option-${init.id}-${o.slice(0, 12)}`} className="rounded-md border border-[#F2B84B]/40 bg-[rgba(242,184,75,0.10)] px-3 py-1.5 text-[11px] font-semibold text-[#F2B84B] transition-colors hover:bg-[#F2B84B]/10 disabled:opacity-50">
        {o}
      </button>
    ));
  };
  ...
  return (
    <article className={`glow-hover rounded-xl border bg-[#0F1D28] p-5 ${traitee ? "border-[rgba(148,163,184,0.16)] opacity-75" : "border-[rgba(148,163,184,0.16)] hover:border-[#41576D]"} transition-colors`} data-testid={`initiative-${init.id}`}>
      <div className="flex items-center gap-2 font-code text-[9px] uppercase tracking-[0.2em]">
        ...
        <span className="ml-auto text-[#7C93A8]">Urgence : {init.urgence}</span>
      </div>

      <h3 className="mt-2 text-base font-semibold leading-snug text-[#F2F6F8]" data-testid={`init-titre-${init.id}`}>{init.titre}</h3>
      ...
      {init.travail_titre && !traitee && (
        <p className="mt-3 flex items-center gap-1.5 font-code text-[10px] text-[#94A3B8]" data-testid={`init-travail-${init.id}`}>
          <FolderOpen size={11} /> Travail existant correspondant : « {init.travail_titre} »
        </p>
      )}

      <div className="mt-3.5 border-t border-[rgba(148,163,184,0.10)] pt-3" data-testid={`init-actions-${init.id}`}>
        {traitee ? (
          <p className="flex items-center gap-1.5 text-xs text-[#34D399]" data-testid={`init-reponse-${init.id}`}>
        ...
        {rejet && !traitee && (
          <div className="mt-2 flex gap-1.5">
            <input value={motif} onChange={(e) => setMotif(e.target.value)} placeholder="Motif du rejet (conservé pour l'apprentissage)…" data-testid={`init-motif-${init.id}`} className="h-8 w-full rounded-md border border-[rgba(148,163,184,0.16)] bg-[#0F1D28] px-2.5 text-xs text-[#F2F6F8] placeholder:text-[#7C93A8] focus:border-[#F87171]/50 focus:outline-none" />
            <button onClick={() => repondre("Rejeter la proposition", { motif })} disabled={envoi || !motif.trim()} data-testid={`init-rejeter-confirm-${init.id}`} className="shrink-0 rounded-md bg-[#F87171] px-3 py-1.5 text-[11px] font-semibold text-[#071019] transition-colors hover:bg-[#991B1B] disabled:opacity-40">
              Rejeter
            </button>
          </div>
        )}
