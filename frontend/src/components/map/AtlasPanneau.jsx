...
export default function AtlasPanneau({
  ...
  // Toute nouvelle sélection rouvre le panneau (sans jamais déplacer la carte)
  useEffect(() => {
    if (comparaison || selectedRelation || selected || domaineSel) { setOuvert(true); setReplie(false); }
  }, [comparaison, selectedRelation, selected, domaineSel]);

  const titre = selected?.nom || domaineSel || (selectedRelation ? "Relation" : comparaison ? "Comparaison" : "Détail");
