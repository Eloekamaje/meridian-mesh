...
export default function CatalogueTiroir({ catalogue, sources, onAjoute, onImport, onFermer }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[15vh]" data-testid="catalogue-tiroir">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.15 }} className="absolute inset-0 bg-white backdrop-blur-sm" onClick={onFermer} />
      <motion.div
        initial={{ opacity: 0, scale: 0.96, y: -8 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        transition={{ duration: 0.18, ease: "easeOut" }}
        className="relative flex max-h-[62vh] w-[min(620px,92vw)] flex-col overflow-hidden rounded-xl border border-[#E5E5E3] bg-white/95 shadow-2xl"
      >
        <div className="flex items-center gap-2 border-b border-[#E5E5E3] px-4 py-3">
          ...
          <button onClick={onFermer} data-testid="catalogue-fermer" className="text-[#71716D] transition-colors hover:text-[#111110]"><X size={16} /></button>
        </div>
