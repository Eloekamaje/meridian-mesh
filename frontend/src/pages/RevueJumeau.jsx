...
export default function RevueJumeau() {
  ...
  const st = jumeau ? STATUTS[jumeau.statut] || [jumeau.statut, "#7C93A8"] : null;
        ...
        <div className="mx-auto max-w-4xl">
          {erreur && (
            <div className="rounded-xl border border-[#F87171]/25 bg-[#F87171]/[0.05] px-5 py-6" data-testid="revue-erreur">
              ...
              <button onClick={() => navigate("/jumeaux")} className="mt-3 rounded-md border border-[rgba(148,163,184,0.16)] px-3 py-1.5 text-xs text-[#94A3B8] transition-colors hover:text-[#F2F6F8]" data-testid="revue-erreur-retour-btn">
                ← Retour aux jumeaux
              </button>
            </div>
          )}
          {!erreur && !examen && (
            <p className="font-code text-[11px] text-[#7C93A8]" data-testid="revue-chargement">Examen du jumeau en cours…</p>
          )}
          {examen && jumeau && (
            <>
              <RevueContenu examen={examen} jumeau={jumeau} />
              {(jumeau.gouvernance || []).length > 0 && (
                <section className="mt-6 rounded-xl border border-[rgba(148,163,184,0.16)] bg-[#0F1D28] p-5" data-testid="revue-gouvernance">
                  <h2 className="font-code text-[10px] uppercase tracking-[0.25em] text-[#94A3B8]">Gouvernance de l'autonomie</h2>
                  <p className="mt-1 text-[11px] text-[#7C93A8]">L'autonomie dépend du type d'action — jamais globale.</p>
                  <div className="mt-3 space-y-1">
                    {jumeau.gouvernance.map((g) => {
                      const c = { delegue: "#34D399", supervise: "#F2B84B", interdit: "#F87171", observe: "#7C93A8" }[g.niveau] || "#7C93A8";
                      const l = { delegue: "Délégué", supervise: "Supervisé", interdit: "Interdit", observe: "Observé" }[g.niveau] || g.niveau;
                      return (
                        <div key={g.action} className="flex items-center justify-between border-b border-[rgba(148,163,184,0.10)] py-1.5 last:border-b-0" data-testid={`gouv-${g.action.slice(0, 20)}`}>
                          <span className="text-xs text-[#D8E2EA]">{g.action}</span>
