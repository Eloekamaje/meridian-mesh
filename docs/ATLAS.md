...
Limites : les observations des vérifications viennent de l'API (démonstration : amorcées en base) — le moteur de raisonnement réel devra les fournir ; l'équipe s'entend comme « l'espace courant » (`espace` du travail) tant que le modèle d'organisation ne définit pas d'équipes ; il n'y a pas encore de délégation à une personne nommée ; les propositions déjà suivies avant cette version n'ont pas de vérification.
