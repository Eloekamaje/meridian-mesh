...
**Accent bleu.** Le style de hubstechs.com a été essayé (polices, angles vifs, rouge, capitales) puis abandonné : Méridian garde son style. Seule la couleur bleue du site est retenue : `Layout` porte `data-accent="bleu"`, et le violet des classes utilitaires (`#9B87F5`, `#B4A5F7`, `#C4B5FD`, `#A78BFA`) devient le bleu doux `#60a5fa` (règles en fin de `index.css`). Les couleurs pilotées par des données (domaines, états de lien, verbes de situation) ne changent pas. Retirer l'attribut rétablit le violet.
