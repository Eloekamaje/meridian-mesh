...
**Peau « hub ».** L'Atlas (rendu graphe) porte `data-skin="hub"` : le style de hubstechs.com — fond `#020617` avec halos bleu/rouge discrets et trame de points bleutée, surfaces `#0b1220`, filets 1 px, angles vifs, titres Outfit léger, texte Manrope, étiquettes JetBrains Mono en capitales espacées, action principale rouge `#e3000f` à coin coupé, cadres à coins sur la légende et les infobulles. Les variables (`--hub-*`) et les règles sont dans `index.css` ; étendre à une autre page = poser le même attribut sur sa racine. Les couleurs de domaine et d'état de lien restent sémantiques et ne changent pas.
