...
**Un seul mode de rendu, à tout zoom — jamais une grappe qui en tient lieu.** (`backend/pyramide.py`.) Après l'essai « essaim de points » (une amélioration, mais qui restait une forme dessinée à la place des jumeaux), le choix est allé plus loin : `GET /api/mesh/vue` ne renvoie plus jamais de grappes. Une vue, c'est toujours `jumeaux` + `liens` — de VRAIES positions, à tout zoom. Ce qui change avec le zoom n'est plus un TYPE de forme, seulement COMBIEN de jumeaux on montre :

- s'ils tiennent tous dans le budget (`MAX_JUMEAUX` = 20 000), on les montre tous, avec leurs vrais liens ;
- sinon, on garde les MIEUX CONNECTÉS d'abord (`ordre_priorite`, un rang stable calculé une fois à la construction — degré, puis un hachage déterministe de l'indice pour départager les ex-æquo — jamais le hasard, qui ferait grouiller la sélection d'une requête à l'autre).

Le rang étant global et stable, **un jumeau une fois montré ne redevient jamais invisible en zoomant** : zoomer réduit la fenêtre, donc la concurrence pour le budget, donc des jumeaux avant exclus (moins bien connectés) y trouvent leur place — le maillage se REMPLIT continûment, il ne « saute » jamais d'un rendu à l'autre (vérifié : `test_zoomer_ne_fait_jamais_disparaitre_un_jumeau_deja_montre`). Le dessin (point minuscule loin, robot reconnaissable près) est une décision du CLIENT, continue avec le zoom (`SEUIL_SPRITE` dans `AtlasEchelle.jsx`) — jamais une seconde décision du serveur. Un double-clic vole, en continu, jusqu'au point visé (même geste que la recherche).

Coût : sélectionner par priorité sans jamais parcourir tout *n* — un raccourci reconnaît qu'une fenêtre englobe déjà tout le Mesh connu (inutile, alors, de le scruter pour le savoir) et va directement chercher, dans l'ordre de priorité global, les `MAX_JUMEAUX` premiers qui tombent dedans (`SCRUTIN_PRIORITE` borne le pire cas). Mesuré à 5 millions de jumeaux, 150 domaines : 2 à 130 ms selon le zoom ; le cas extrême d'une fenêtre couvrant l'intégralité du Mesh (rare en usage réel — le cadrage initial ne s'y met jamais) reste autour de 350 ms.

La hiérarchie interne domaine → groupe → communauté (Louvain) n'a pas disparu : elle sert toujours le RÉSUMÉ (légende par domaine, effectifs, écarts déclaré ≠ calculé), seulement plus jamais à dessiner une bulle à la place des jumeaux qu'elle contient.

**Domaine déclaré (BCM) vs communauté structurelle (calculée) : un vrai désaccord de GROUPE, pas seulement individuel.** (`backend/pyramide.py::synthetique`.) La POSITION d'un jumeau vient toujours de sa communauté (structurelle, gaussienne autour d'un centre organique) — jamais de son seul domaine déclaré. Environ un cinquième des communautés sont volontairement SCINDÉES entre deux domaines déclarés (une part substantielle, 25 à 50 % de la communauté, pas un jumeau isolé) : c'est cet écart-là — une équipe organisée autrement que l'organigramme officiel — que l'Atlas est censé donner à voir, pas seulement du bruit statistique individuel. Chaque domaine garde toujours au moins une communauté qui lui appartient PLEINEMENT (jamais scindée) : même à des centaines de domaines, aucun ne se retrouve à zéro membre par le seul jeu du hasard. Conséquence acceptée : le territoire calculé de deux domaines peut légèrement se chevaucher (jusqu'à ~8 %, vérifié) — les jumeaux d'une communauté scindée sont comptés dans le domaine qui les a reçus, où qu'il se trouve dans l'amas.

**Les liens aussi se résument, loin — puis redeviennent réels en zoomant.** (`backend/pyramide.py` : `_liens_agreges`, `_niveau_liens_agreges`.) Des milliers de liens réels dessinés un par un ne seraient qu'une pelote illisible — pas plus d'information qu'un trait unique, pondéré par leur VRAI nombre, entre les deux zones qu'ils relient. `GET /api/mesh/vue` renvoie alors, EN PLUS des vrais liens (toujours présents, entre les jumeaux affichés), un `liens_agreges` : un trait par paire de grappes (communauté, groupe ou domaine — le niveau se resserre avec le zoom), jamais inventé — un vrai décompte, juste résumé. Seuls les ARCS se résument : les NŒUDS restent toujours réels (aucun rapport avec l'ancienne agrégation en grappes, retirée). Zoomer fait DISPARAÎTRE l'agrégat en douceur (`fondu_agreges`, de 1 à 0 sur une bande de zoom, jamais un saut) pendant que les vrais liens, déjà envoyés, prennent le relais à mesure qu'ils deviennent lisibles seuls — jusqu'à ce que l'agrégat ne soit plus renvoyé du tout. Rendu (`AtlasEchelle.jsx`) : les arcs sont de légères courbes (jamais des droites qui se croisent en étoile), leur opacité et leur épaisseur suivent le poids réel ; les vrais liens s'estompent à `1 − fondu_agreges` pendant la transition, pour ne jamais se superposer à pleine force à l'agrégat qu'ils remplacent.

**Arcs discrets, communautés au premier plan.** Les arcs agrégés étaient trop présents — ils faisaient concurrence aux points plutôt que de rester un fond suggestif. Opacité et épaisseur revues nettement à la baisse (`AtlasEchelle.jsx` : plafond d'opacité 0,27 au lieu de 0,6, épaisseur max ~2,2 px au lieu de ~3,8) : au dézoom maximal, on voit d'abord les galaxies de points colorés, les arcs ne sont qu'une suggestion de trafic en arrière-plan.

**Illumination des connexions : persistante à la sélection, pas seulement au survol.** Dans le laboratoire, cliquer un jumeau (ouvrir son panneau) éteignait l'illumination de ses connexions dès que la souris quittait le point — elle ne suivait que le survol. Un jumeau synthétique n'ayant pas d'identifiant réel (`id`), le rapprochement entre le jumeau sélectionné et l'affichage courant échouait silencieusement. `AtlasEchelle` accepte maintenant un second repère, `selectionIndex` (le numéro `i`, toujours présent, contrairement à `id` qui n'existe que pour le vrai Mesh) ; `GET /api/mesh/vue/jumeau` le renvoie dans sa réponse. Le focus qui pilote l'illumination (jumeau et liens en évidence, le reste atténué) vient maintenant du survol OU de la sélection — même comportement que l'Atlas réel : les connexions restent visibles tant que le panneau est ouvert, qu'on pointe dessus ou non.

**Le clic vole aussi jusqu'au jumeau.** Cliquer un jumeau n'ouvrait que son panneau : la caméra restait où elle était, même si le point cliqué n'était qu'un pixel minuscule au loin. `AtlasEchelle.jsx` réutilise maintenant `voler` (le même vol continu que le double-clic et la recherche) au clic : la caméra se centre sur le jumeau choisi et atteint un zoom minimal garanti-lisible (`42 / (échelle × 1,9)` — assez pour distinguer un robot et son identifiant), sans jamais DÉZOOMER si on était déjà plus proche que ce seuil. Un « saut » de vue reste exclu par construction : c'est la même interpolation géométrique du zoom qu'ailleurs dans le composant, jamais un repositionnement instantané.
