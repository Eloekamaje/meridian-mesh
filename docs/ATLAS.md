...
**Peau « hub ».** Toute la coquille (`Layout` : barre latérale, pages, panneau Flore) porte `data-skin="hub"` : le style de hubstechs.com — fond `#020617` avec halos bleu/rouge discrets et trame de points bleutée, surfaces `#0b1220`, filets 1 px, angles vifs, titres Outfit léger, texte Manrope, étiquettes JetBrains Mono en capitales espacées, action principale rouge `#e3000f` à coin coupé, cadres à coins sur la légende et les infobulles. Les variables (`--hub-*`) et les règles sont dans `index.css` ; les fonds en dur (`#071019`, `#091420`, `#0C1724`…) sont ramenés aux surfaces du site, l'accent violet des classes utilitaires (`#9B87F5`, `#B4A5F7`, `#C4B5FD`) devient le bleu doux `#60a5fa`, la navigation passe en capitales espacées, « Nouveau travail » en contour rouge. Les couleurs de domaine et d'état de lien, et les couleurs pilotées par des données (états, verbes de situation), restent sémantiques et ne changent pas. Retirer la peau = retirer l'attribut de `Layout.jsx`.
