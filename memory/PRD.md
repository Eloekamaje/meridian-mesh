...
- **`AreteCorridor.jsx`** : rendu du trajet via `construireD(points, [], 64)` — les coudes 90° deviennent des courbes organiques (« voies hyper-espace ») ; secours = ancien arc quadratique entre capitales si routage indisponible ; états survol/illumination/pointillés d'activité inchangés.
- Vérifié par captures : au Global les corridors touchent les bords des membranes et contournent les territoires tiers (canaux orthogonaux arrondis), zéro traversée visible, zéro erreur de compilation.
...
- **Constellations territoriales** (`lib/constellation.js` + `RegionNode.jsx`) : 20–64 micro-points échantillonnés dans la coque concave (rejet dans polygone, PRNG ensemencé par id de domaine → rendu stable), reliés par filaments plus-proche-voisin (+ 2ᵉ voisin < 95 px à 45 %), 10 % d'étoiles brillantes avec halo, 16 % scintillent. Membrane atténuée : remplissage 11 % → 5 %, contour renforcé au survol conservé (affordance + identité couleur).
- **Relations courbes aux niveaux 1-2** (`AreteOrthogonale.jsx`) : arc quadratique source→cible, courbure perpendiculaire déterministe (sens alterné par hachage d'id, max 54 px) ; couleurs sémantiques conservées (cyan observée / ambre pointillés supposée / violet validation / ardoise confirmée) ; marqueurs-chevilles désactivés en mode courbe. Niveau 3+ : routage orthogonal libavoid inchangé. Corridors niveau 1 : `smoothstep` → bézier (`type: "default"`), trait cyan lumineux fin.
- Tests : **iteration_56 → 100 %** (6/6 critères : étoiles N2, zoom sémantique aller-retour sans artefact, toutes interactions — lasso, recherche, Flore, panneaux, réorganisation avec persistance — ciel fixe au pan, 0 erreur console).
...
- **Rétraction en loupe** : quand le panneau détail OU Flore est ouvert, le champ devient un bouton loupe (`btn-recherche`) ; clic = re-dépliage à la demande (`loupeForcee`, bouton X pour refermer) ; la fermeture des panneaux restaure le champ complet automatiquement.
- **Flore = colonne de layout** (`FlorePanel` passe de `fixed` à `flex h-full w-[440px] shrink-0`, enfant du flex racine de `Layout`) : sur TOUTES les pages, le contenu se redimensionne — sur l'Atlas la carte rétrécit exactement comme avec le panneau détail (mini-carte et zoom toujours visibles, caméra conservée par le ResizeObserver). Mobile ≤ 640 px : overlay plein écran conservé (`max-sm:fixed`). Le pan contextuel Flore (220 px) est supprimé d'Atlas. Flore reste bloquée sur /travaux (design v47).
- **Re-bornage automatique** (`useGlissable`) : un ResizeObserver sur le conteneur ET l'élément re-clampe l'offset persisté dès que la géométrie change (colonnes, fenêtre, dépliage du tiroir) — une barre/panneau déplacé ne peut plus jamais sortir du champ ni passer sous une colonne (cause du bug « je ne vois plus la barre d'outils » : offset persisté + carte rétrécie = élément clippé par `overflow-hidden`). Debounce 350 ms (> transition d'ancrage 300 ms).
- Tests : **iteration_53 → 100 % (9/9)** : recherche à gauche, loupe (rétraction/re-dépliage/fermeture/restauration), Flore colonne seule et combinée au panneau détail, autres pages sans débordement, blocage /travaux conservé, chrome immobile, restauration navigation, mobile. Re-bornage auto-vérifié Playwright (offset extrême 3000px → ramené dans les bornes ; offset modéré + 2 colonnes → barre visible).
...
- **Suppression du regroupement (grappes)** (choix utilisateur) : la logique `clusteriser` est désactivée et le rendu des nœuds « ×N » est retiré de `TwinNode.jsx`. Tous les jumeaux s'affichent individuellement, quel que soit le niveau de zoom ou la densité du domaine. L'algorithme anti-collision garantit l'absence de chevauchement.
